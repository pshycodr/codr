import { Project, Node, SourceFile, FunctionDeclaration, ArrowFunction, MethodDeclaration } from 'ts-morph';
import fs from 'fs';
...
function extractFunctionDeclarations(sourceFile: SourceFile): Record<string, string> {
  const functionMap: Record<string, string> = {};

  sourceFile.forEachDescendant(desc => {
    if (Node.isFunctionDeclaration(desc) || Node.isArrowFunction(desc) || Node.isMethodDeclaration(desc)) {
...
function generateCallGraph() {
  ...
  for (const edge of edges) {
    ...
    const calleeKey = calleeFile ? `${calleeFile}::${edge.callee}` : `${edge.filePath}::${edge.callee}`;
  ...
  const allKeys = new Set([...Object.keys(callsMap), ...Object.keys(calledByMap)]);
