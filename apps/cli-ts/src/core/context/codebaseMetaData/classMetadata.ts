import { Project, SyntaxKind, SourceFile, PropertyDeclaration, MethodDeclaration } from 'ts-morph';
import fs from 'fs';
...
function extractClassMeta(sourceFile: SourceFile): ClassMeta[] {
  ...
  sourceFile.getClasses().forEach(cls => {
    ...
    const classMeta: ClassMeta = {
      ...
      properties: props,
      methods: methods,
      docstring: cls.getJsDocs().map(doc => doc.getComment()).filter(Boolean).join('\n'),
    };
