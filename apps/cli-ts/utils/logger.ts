...
const formatConsoleLog = (level: string, message: string): string => {
	...
	switch (level) {
		case "info":
			return chalk.blue(prefix) + " " + chalk.white(message);
		case "warn":
			return chalk.yellow(prefix) + " " + chalk.yellow(message);
		case "error":
			return chalk.red(prefix) + " " + chalk.redBright(message);
		case "debug":
			return chalk.gray(prefix) + " " + chalk.dim(message);
		case "time":
			return chalk.green(prefix) + " " + chalk.greenBright(message);
		default:
...
function logWithConsole(
	level: "info" | "warn" | "error" | "debug" | "time",
	message: string,
...
// Main logger object
const logger = {
	info: (msg: string, printToConsole = false) =>
		logWithConsole("info", msg, printToConsole),
	warn: (msg: string, printToConsole = false) =>
		logWithConsole("warn", msg, printToConsole),
	error: (msg: string, printToConsole = false) =>
		logWithConsole("error", msg, printToConsole),
	debug: (msg: string, printToConsole = false) =>
		logWithConsole("debug", msg, printToConsole),
